#!/usr/bin/env python3
"""dots.py - Compact state management DSL for AI-assisted projects.

Usage:
    dots init                  Scaffold dotS/ structure
    dots get <file> [block]    Read file or specific block
    dots get index.s *         Load index + ALL skills (quote * or it may expand)
    dots set <file> <path> <value>  Set a value (path: block.key or block.key.subkey)
    dots add <file> <path> <value>  Append to a list
    dots list <file>           List all blocks in a file
    dots graph                 Show relationship graph
    dots validate              Validate all .s files
    dots snap                  Snapshot current state
    dots diff                  Show changes since last snapshot
    dots search <query>        Search across all .s files
    dots find <topic>          Smart lookup via index.s routing
    dots optimize <skill> [--dry-run]  Optimize .s files for token savings
"""
...
def _auto_load_skills_from_index(index_sf: SFile):
    """Auto-load all skilldots listed in @index block of index.s."""
...
def _is_load_all(args: list) -> bool:
    ...
    Matches 'dots get *' and 'dots get index.s *'. When the shell expands
...
def cmd_get(args):
    ...
    if not args:
        print("Usage: dots get <file> [block|*]", file=sys.stderr)
        sys.exit(1)

    # Wildcard shortcuts (quote the * or rely on expansion tolerance):
    #   dots get index.s *   -> load every skill in skills/
    #   dots get *           -> same
...
def cmd_set(args):
    ...
    if len(args) < 3:
        print("Usage: dots set <file> <block.key> <value>", file=sys.stderr)
        sys.exit(1)
...
def cmd_add(args):
    ...
    if len(args) < 3:
        print("Usage: dots add <file> <block.key> <value>", file=sys.stderr)
        sys.exit(1)
...
def cmd_list(args):
    ...
    if not args:
        print("Usage: dots list <file>", file=sys.stderr)
        sys.exit(1)
...
def cmd_diff(args):
    ...
    if not snaps:
        print("  No snapshots yet. Run 'dots snap' first.")
        return
...
def cmd_search(args):
    ...
    if not args:
        print("Usage: dots search <query>", file=sys.stderr)
        sys.exit(1)
...
def cmd_find(args):
    ...
    if not args:
        print("Usage: dots find <topic>", file=sys.stderr)
        sys.exit(1)
...
def cmd_learn(args):
    ...
    if not filepath.exists():
        print(f"  file not found: {filepath}", file=sys.stderr)
        print(f"  create it first: dots set {args[0]} @meta topic {args[0].replace('.s','')}")
        sys.exit(1)
...
def cmd_optimize(args):
    ...
    Usage: dots optimize <skill> [skill2] ... [--dry-run]
           dots optimize --dry-run css html nginx
    ...
    if not skill_names:
        print("Usage: dots optimize <skill> [skill2] ... [--dry-run]", file=sys.stderr)
        sys.exit(1)
...
def cmd_help(args):
    ...
    if args and args[0] in COMMANDS:
        ...
        print("  dots.py - compact state DSL for AI-assisted projects")
        print(f"\n  commands:")
        for name, cmd in COMMANDS.items():
            print(f"    {name:12s} {(cmd.__doc__ or '').strip().splitlines()[0]}")
        print(f"\n  dotS dir: {CTX_DIR}")
        print(f"\n  examples:")
        print(f"    dots get index.s           # read project index")
        print(f"    dots get index.s *         # load index + ALL skills")
        print(f"    dots get modules/core.s @t # read TODOs from core module")
        print(f"    dots set modules/core.s s.state stable")
        print(f"    dots add modules/core.s @t.add 'TODO: thing' priority:high")
        print(f"    dots graph                 # show relationships")
        print(f"    dots search 'todo'         # search across all files")
...
def cmd_deps(args):
    ...
    Usage: dots deps <skill>
           dots deps blender-python
           dots deps --all
    """
    if not args:
        print("Usage: dots deps <skill> | --all", file=sys.stderr)
        sys.exit(1)
    ...
    if args[0] == 'list':
        ...
        for f in sorted(skills_dir.glob("*.s")):
            if f.name.startswith('.'):
                continue
            sf = SFile(f)
            blocks = [b.tag for b in sf.blocks if not b.tag.startswith('_') and b.tag not in SKIP]
            if blocks:
    ...
    if args[0] == '--all':
        ...
        for i, s1 in enumerate(skills):
            for s2 in skills[i+1:]:
                ...
                blocks1 = set(b.tag for b in sf1.blocks) - SKIP
                blocks2 = set(b.tag for b in sf2.blocks) - SKIP
...
def cmd_run(args):
    ...
    Usage: dots run git.s @run.quickCommit
           dots run nginx.s @run.sslSetup --dry-run
    """
    import subprocess
    
    if len(args) < 2:
        print("Usage: dots run <file.s> <@run.blockName> [--dry-run]", file=sys.stderr)
        sys.exit(1)
