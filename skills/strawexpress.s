...
@architecture |
  ...
  flow:|
    URL hash change
      -> AddressChanger detects hashchange
        -> Hierarchy.redistribute(path)
          -> Hierarchy.formulate(path) => Command[]
            -> CommandQueue executes commands
              -> Steps open/close via commandOpen/commandClose
                -> Response handlers render Jade templates via AJAX
...
@classes |
  Kompat:Lines 44-181
    purpose:browser/OS compatibility detection singleton
    methods:getInstance checkBrave checkES6BaseCompliant initialize getName getVersion getOs getOsVersion
  CodeUtil:Lines 188-207
    methods:overwritesafe(target propname propvalue) deepclone(o)
  StringUtil:Lines 211-234
    constants:SPACE SLASH HASH AROBASE DOLLAR EMPTY
    extends:String.prototype.test() for multi-string regex
  NumberUtil:Lines 237-248
    extends:Number.prototype.clamped(min max)
  ArrayUtil:Lines 252-286
    methods:isArray(obj) argsToArray(args) indexOf(arr obj)
  PathUtil:Lines 327-419
    methods:trimlast trimfirst trimall ensurelast ensurefirst ensureall endslash startslash allslash eitherslash removeMultipleSeparators replaceUnderscores
...
@events |
  IEvent:Lines 881-898
    ...
    purpose:lightweight event object type can be string or another IEvent to clone
  DOMEventDispatcher:Lines 902-968
    methods:trigger(e) bind(type closure) unbind(type closure) destroy
  EventDispatcher:Lines 974-1047
    extends:DOMEventDispatcher
    methods:setDispatcher(tg) bind unbind trigger fire willTrigger willTriggerNow destroy
...
@commands |
  Command:Lines 2039-2081
    ...
    signature:Command(thisObj closure ...params)
    methods:execute dispatchComplete cancel destroy
    async:truthy return = sync falsy = async
  CommandQueue:Lines 2087-2206
    extends:Command
    signature:CommandQueue(...commands)
    methods:add(next execute alias reset cancel destroy
  ...
  WaitCommand:Lines 2211-2275
    signature:WaitCommand(time initclosure?)
    methods:execute always returns this async cancel destroy
...
@backgroundQueue |
  BackgroundTaskQueue:Lines 4237-4441
    ...
    tiers:|
      Tier 0 critical:runs synchronously at start() sync tasks no done callback
      Tier 1 background:runs one task per frame via BJS.timeout(0) tasks receive done() callback
    ...
    usage:|
      ...
      q.add(function(done) { compileShader(bg, done); }, 1);
      q.add(function(done) { loadPointCloud(done); }, 1);
      q.start();
    quirks:|
      tier 0 tasks must be synchronous no done callback
      tier 1 tasks must call done() to advance to next frame
...
@step |
  Step:Lines 2612-3065
    purpose:a node in the address hierarchy tree representing a page/route
    signature:Step(id commandOpen commandClose)
    openLifecycle:|
      1. Guard throw if already opened and not closing
      2. Set opening = true
      3. If isOpenable execute commandOpen
      4. Dispatch step_opening
      5. If async bind completion then checkOpenNDispatch
      6. opened = true opening = false dispatch step_open
    closeLifecycle:|
...
@address |
  ...
  AddressHierarchy:Lines 3757-3835
    extends:Hierarchy
    purpose:singleton URL hash-based hierarchy with locale support
    methods:sliceLocale redistribute headTo
...
@express |
  Express:Lines 4243-4521
    purpose:the main SPA singleton app
    routeRegistration:|
      get(pattern handler parent?) register route
      get(key) settings getter 1 arg
      handler receives(req res) where res is Response instance
