...
@tweenHierarchy |
  AbstractTween:Lines 1101-1654
    inherits:Traceable
    purpose:base tween with full lifecycle
  ...
  Tween:Lines 1656
    ...
    purpose:concrete numeric property tween
  ...
  AbstractActionTween:Lines 1677
    inherits:AbstractTween
    purpose:base for action tweens that execute side effects
  TweenDecorator:Lines 2101
    inherits:AbstractTween
    purpose:base for decorators wrapping a baseTween
  GroupTween:Lines 2450
    inherits:AbstractTween
    purpose:base for parallel/serial groups
  ParallelTween:Lines 2571
    inherits:GroupTween
    purpose:plays child tweens simultaneously
...
@actions |
  FunctionAction:Lines 1724
    purpose:executes a function at completion supports rollback
  TimeoutAction:Lines 1777
    purpose:fires once after duration wraps FunctionAction
  IntervalAction:Lines 1804
    purpose:fires repeatedly at intervals auto-resets startTime
  LoadAction:Lines 1859
    purpose:XHR loader with local caching blocks serial chain until loaded
    cache:BetweenJS.filescache same URL served from cache unless forceBrowserNoCache
  AnimationFrameAction:Lines 1982
    purpose:starts continuous rAF-based animation loop within serial chain
  AddChildAction:Lines 2021
    purpose:DOM appendChild action with rollback removeChild
...
@decorators |
  SlicedTween:Lines 2161
    purpose:plays [begin end] portion of base tween supports percent or absolute
  ScaledTween:Lines 2245
    purpose:multiplies base tween duration by scale
  ReversedTween:Lines 2305
    purpose:reverses playback direction double-reverse returns original idempotent
  RepeatedTween:Lines 2342
    purpose:repeats base tween repeatCount times
...
@updaters |
  UpdaterFactory:Lines 2828
    purpose:factory with object pooling routes options to createBasic/createAction/createDecorator/createGroup
  Updater:Lines 3226
    purpose:core interpolation engine handles source/dest/cuepoints physical ease duration factor calculation
  ...
  UpdaterProxy:Lines 3621
    purpose:proxies nested property updates e.g. transform sub-properties like translateX
...
@easing |
  ...
  parameterized:|
    ...
    Back:easeInWith(s) easeOutWith(s) easeInOutWith(s) easeOutInWith(s)
